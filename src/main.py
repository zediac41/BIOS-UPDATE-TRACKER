...
def parse_versions_asus(html):
    ...
    versions = []
    for tag in soup.find_all(["td", "div", "span", "a", "li"]):
        text = tag.get_text(strip=True)
        if not text:
            continue
        t = text.lower()
        if "bios" in t or t.replace(".", "").replace("v", "").isalnum():
...
def parse_versions_gigabyte(html):
    ...
    for tag in soup.find_all(["li", "td", "div", "span", "a"]):
        ...
        if not text:
            continue
        t = text.lower()
        if "bios" in t or t.replace(".", "").replace("f", "").replace("v", "").isalnum():
            versions.append(text)
    return versions
...
def parse_versions_asrock(html):
    ...
    for tag in soup.find_all(["td", "div", "span", "a", "li"]):
        ...
        if not text:
            continue
        t = text.lower()
        if "bios" in t or t.replace(".", "").replace("p", "").replace("v", "").isalnum():
            versions.append(text)
    return versions
